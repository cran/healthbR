...
The package currently supports **16 data sources** organized in three groups:
...
ANS (Agencia Nacional de Saude Suplementar) provides open data on private
health insurance in Brazil, including beneficiary counts, consumer complaints,
and operator financial statements.
...
See `vignette("ans-supplementary-health")` for detailed examples.

## ANVISA -- Medicines and pharmacovigilance

ANVISA (Agencia Nacional de Vigilancia Sanitaria) publishes open data on
regulated products (medicines, medical devices, food, cosmetics) and
pharmacovigilance reports (VigiMed). The SNGPC module provides controlled
substance dispensing data.
...
See `vignette("anvisa-regulatory-data")` for detailed examples.
...
When downloading data for multiple years, months, or states, healthbR processes
each file sequentially by default. You can enable parallel downloads by
installing `furrr` and `future` and setting a parallel plan before calling any
`*_data()` function:
...
This works with every module that downloads multiple files, including SIM,
SINASC, SIH, SIA, SINAN, CNES, SI-PNI, SISAB, ANS, ANVISA, PNS, and PNADC.
No code changes are needed beyond setting the plan -- healthbR detects the
active `future` plan automatically.

A few practical notes:

- **FTP sources** (DATASUS modules) include a small delay between requests to
  avoid overwhelming the server. Parallel workers respect this delay
  independently, so 4 workers download roughly 4x faster.
- **SI-PNI CSV** (2020+) uses an additional concurrent HTTP strategy
  (`curl::multi_download()`) that works without `furrr`, downloading multiple
  months simultaneously within a single R session.
- Choose the number of workers based on your connection and the server. 2--4
  workers is usually a good balance between speed and politeness.
...
- `vignette("sipni-vaccination")` -- SI-PNI vaccination data (FTP + API)
- `vignette("sisab-primary-care")` -- SISAB primary care coverage indicators
- `vignette("ans-supplementary-health")` -- ANS supplementary health data
- `vignette("anvisa-regulatory-data")` -- ANVISA medicines and pharmacovigilance
